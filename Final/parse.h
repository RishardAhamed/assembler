...
##########################################################################################################
##########################################################################################################
#################################### process.c/h General Overview: ##################################
##########################################################################################################
##########################################################################################################

This file purpose is to be the main file that is responsible for parsing of the expanded assembly code.
while parsing the code in this file we are invoking different functions that exists in the firstround.c
and the secondround.c depending on the global state of our assembler (firstround/secondround).
...
/* @ Function: parseLine
   ...
   @ Description: The function checks what is the current globalState, than checks what is the first token (an instruction, an operation, a label declaration...)
    If the globalState is equal to firstround, according to the first token, it finds out what kind of line is the current line and calls other function so they can handle the line.
    If the globalState is equal to secondround, according to the first token, it calls function to write the compiled memory.
