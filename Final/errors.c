...
void fileCreationFailure(char *fileName)
{
    ...
    if (!isErrorFileExist)
    {
        ...
        {
            fprintf(stderr, "\n###### File Creation Error ######\n");
            fprintf(stderr, " Unable to generate %s error log file\n", fileName);
            fprintf(stderr, "##################################\n\n");
        }
        else
    ...
    fprintf(errorsFile, "\n###### File Creation Error ######\n");
    fprintf(errorsFile, " Unable to create %s file\n", fileName);
    fprintf(errorsFile, "##################################\n\n");
}
...
void yieldWarningIntoFile(Warning err)
{
    ...
    if (!isWarningFileExist)
    {
        if ((warningsFile = fopen("warnings.log", "w+")) == NULL)
            printf("Unable to open warning log file\n");
        else
            isWarningFileExist = True;
    }

    fprintf(warningsFile, "\n--------------------- WARNING ---------------------\n");
    fprintf(warningsFile, "File: %s, Line Number: %d\n", (*filePath)(), (*line)());

    switch (err)
    {
    case emptyDataDeclaretion:
        fprintf(warningsFile, "Data declaration is empty.");
        break;

    case emptyExternalDeclaretion:
        fprintf(warningsFile, "External declaration is empty.");
        break;

    case emptyEntryDeclaretion:
        fprintf(warningsFile, "Entry declaration is empty.");
        break;

    case emptyDeclaretionOfEntryOrExternalVariables:
        fprintf(warningsFile, "Declaration of entry or external variables is empty.");
        break;
...
void yieldErrorIntoFile(Error err)
{
    ...
    {
        if ((errorsFile = fopen("errors.log", "w+")) == NULL)
            printf("Failed to initiate error log file\n");
        else
            isErrorFileExist = True;
    }
    fprintf(errorsFile, "\n<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n");
    fprintf(errorsFile, "Error encountered in file %s on line number %d\n", (*filePath)(), (*line)());

    switch (err)
    {
    case macroDeclaretionWithoutDefiningMacroName:
        fprintf(errorsFile, "Macro name not defined during declaration!");
        break;

    case registeryIndexOperandTypeIfOutOfAllowedRegisteriesRange:
        fprintf(errorsFile, "Illegal use of registry index operand addressing method. Registry Index addr method only allows the use of registries that are between r10 - r15!");
        break;

    case illegalInputPassedAsOperandSrcOperand:
        fprintf(errorsFile, "Unaccepted input provided as source operand.");
        break;

    case illegalInputPassedAsOperandDesOperand:
        fprintf(errorsFile, "Unaccepted input provided as destination operand.");
        break;

    case undefinedLabelDeclaretion:
        fprintf(errorsFile, "Label declaration is undefined.");
        break;
